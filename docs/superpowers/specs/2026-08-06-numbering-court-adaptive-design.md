# Numbering mode — court/bench-adaptive matchmaking

**Date:** 2026-08-06
**Status:** Design approved, ready for implementation plan
**Scope:** Numbering mode only (`mm() === 'random'`). Balanced/other modes untouched.
...
Two call sites in `app.html` plus one addition in `tournament.js`:

1. **`app.html` — new helper** `numberingConfig()` returning `{ beta, decay, K }` from
   `presentPlayers().length` and `courtDefs.length` per the `healthy` rule above. ("Up
   Next" visibility is a separate direct `courtDefs.length >= 4` check at render time.)
2. **`app.html:2242`** (`chooseMatchPlayers`, the `random` branch): pass
   `{ teamSize: ts, beta, decay, K }` from the helper into `fairWeightedMatch`.
3. **`app.html` — "Up Next" render**: hide the on-deck card when `mm() === 'random'`
   and `courtDefs.length < 4`. Leave `MAX_QUEUED` at `1` for Numbering (internal seat
   flow depends on a ready match). Do not touch other modes.
4. **`tournament.js` `fairWeightedMatch`** (split step ~L165): add the recent-partner
   blocker as the primary sort key over the existing split cost. Add a `blockWindow`
   option (default 3); `0` disables (preserves current behavior for callers/tests).

Non-goals: no change to opponent handling (`alpha`), no change to Balanced or other
modes, no change to the draw's core weighted-selection math beyond the K value.

## Testing

- **Unit** (`tests/fair-match*.test.js`): blocker avoids a last-3-games pair when an
  alternative split exists; falls back without error when all splits are blocked;
  `blockWindow: 0` reproduces current output. `numberingConfig()` returns correct
  `{onDeck, beta, decay, K}` at the boundaries (11 vs 12 players; 3 vs 4 courts;
  bench 2 vs 3).
- **Session** (`/test-this-mode`): re-run the four reference scenarios (7p/1ct,
  14p/2ct, 20p/3ct, 39p/4ct) and confirm the evidence numbers hold on the wired path.
